...
async def get_leaderboard(lb_type: str):
    ...
    if lb_type == "coins":
        cursor = await db.execute("SELECT user_id, username, full_name, coins FROM players ORDER BY coins DESC LIMIT 20")
    elif lb_type == "level":
        cursor = await db.execute("SELECT user_id, username, full_name, level, exp FROM players ORDER BY level DESC, exp DESC LIMIT 20")
    elif lb_type == "kills":
        cursor = await db.execute("SELECT user_id, username, full_name, total_kills FROM players ORDER BY total_kills DESC LIMIT 20")
    elif lb_type == "earnings":
        cursor = await db.execute("SELECT user_id, username, full_name, total_earnings FROM players ORDER BY total_earnings DESC LIMIT 20")
    else:
        return []
...
async def get_logs(limit: int = 100, severity: str = None):
    ...
    if severity:
        ...
        cursor = await db.execute("SELECT * FROM activity_logs ORDER BY created_at DESC LIMIT ?", (limit,))
    return await cursor.fetchall()
...
async def get_stats():
    ...
    cursor = await db.execute("SELECT COUNT(*) FROM players WHERE last_active > datetime('now', '-1 hour')")
    stats['online_players'] = (await cursor.fetchone())[0]
    cursor = await db.execute("SELECT SUM(amount) FROM transactions WHERE type='topup' AND status='approved' AND created_at > datetime('now', 'start of day')")
    result = await cursor.fetchone()
...
async def check_achievements(user_id: int):
    ...
    for ach in achievements:
        ...
        if unlocked:
            await db.execute("INSERT INTO player_achievements (user_id, achievement_id) VALUES (?,?)", (user_id, ach['id']))
            await db.execute("UPDATE players SET coins=coins+? WHERE user_id=?", (ach['reward'], user_id))
            await db.commit()
